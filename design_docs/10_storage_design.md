...
| 产品版本 | 存储形态 | 说明 |
|---|---|---|
| 1.0 | 静态 JSON + 公开 demo data | 只读演示，证明完整产品闭环 |
| 2.0 | SQLite + REST API + VPS 本地文件 | 支持真实写入、LLM 结果入库和 job 状态持久化 |
| 3.0 | PostgreSQL/MySQL + 私有 COS | 支持多用户、真实数据量、鉴权访问和备份恢复 |

因此阶段 G 不是独立大版本，而是 2.0 的后端基础设施；阶段 F 在同一存储契约上接入真实 LLM 后，才构成 2.0 的核心 AI 能力。
