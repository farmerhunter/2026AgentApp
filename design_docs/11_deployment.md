...
**Rollup 原生模块缺失**：`npm ci` 在某些环境下可能漏装 `@rollup/rollup-linux-x64-gnu`（已知 npm 可选依赖 bug）。部署脚本已改用 `npm install` 避免此问题。
...
部署脚本 (`scripts/deploy_web_ui.sh`) 通过 `NPM_REGISTRY` 环境变量控制下载源，默认使用官方源。国内 VPS 如需加速，设置：
```bash
NPM_REGISTRY=https://mirrors.tencentyun.com/npm bash scripts/deploy_web_ui.sh
```
脚本在构建完成后会自动用官方源重写 lockfile 并通过 `validate-lockfile.mjs` 校验，确保提交的 lockfile 不含镜像 URL。
