...
- `learning_insight_update`：基于一次或一组学习证据生成局部发现、待确定记忆和行动候选。
...
7. 上传结束页展示 `learning_insight_update` 结果，支持 findings、待确定记忆和行动候选的人工确认。
...
- 任务队列。
- 微信端上传、备注、提醒和简版周报。
- 上传结束页中的 learning findings / 待确定记忆确认。
- 长期方向再扩展独立 learning insight review 或长期记忆管理界面。
