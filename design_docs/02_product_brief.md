...
- 教材分析、学习洞察更新和生成周报按钮通过 static demo job 链路展示任务状态与 sample result，不直接调用真实 API。
...
- 1.0 默认 `VITE_HERMES_EXECUTION_MODE=static`，所有 Hermes 任务按钮通过 sample data 模拟执行，不请求 `/api/hermes/*`。
- 2.0 默认 `HERMES_DATA_SOURCE=api`，`HERMES_STORAGE_PROVIDER=local`，`HERMES_JOB_MODE=fixture|real`。
- 2.0 默认 `VITE_HERMES_EXECUTION_MODE=api`，任务按钮通过 `/api/hermes/jobs` 创建和轮询真实 job。
