...
3. 根据固定 skill 和 prompt 重复执行教材摘要、重点题总结、周报生成等任务。
...
src/prompts/
  ...
  focus_question_record.prompt.md
  weekly_report.prompt.md
  study_plan.prompt.md
```
...
### 3.2 `question_capture`

用途：基于腾讯云试卷切题结果和学生人工确认结果，生成重点题记录。
...
用途：从周报、重点题和学生备注中生成下一步学习行动建议。

第一版可以并入 `weekly_report` 输出，不单独做真实 job。长期阶段再拆成独立计划生成任务。
