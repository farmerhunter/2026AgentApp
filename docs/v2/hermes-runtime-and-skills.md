...
本周不同题目之间有证据的共性、重复和变化，在这一次报告任务内部综合分析。周报 2.0 由应用提供权威的日期、练习数、错题数和证据目录；Hermes 只返回总览、1–3 个重点、最多 1 个观察项和 1–2 个下一步行动，并使用 `evidence_ref` 引用依据。应用在保存前检查数量、证据引用、重复/改善成立条件、文案长度和内部 ID 泄漏，再附加可读证据详情。页面先展示结论和行动，题目与作答默认折叠，按重点展开。周报不产生记忆候选或自动改写已有 findings，也不新增独立聚合 Job 或问答 UI。
