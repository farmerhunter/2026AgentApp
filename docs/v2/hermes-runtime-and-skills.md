...
**状态：Current design / 周报 2.0 已完成本地实现，待真实 Hermes 内容验收。** 本文单独说明 Hermes 在 V2 中为什么存在、如何接入，以及如何高效迭代 Skill。
...
本周不同题目之间有证据的共性、重复和变化，在这一次报告任务内部综合分析。周报 2.0 由应用提供权威的日期、练习数、错题数和证据目录；Hermes 只返回总览、1–3 个重点、最多 1 个观察项和 1–2 个下一步行动，并使用 `evidence_ref` 引用依据。Skill 的目标字数低于保存硬门禁；模型偶尔写长时，应用只在已有标点处分句收短，不改证据引用、数字或算式。原始面向用户文案仍先经过内部 ID 检查，收短后的结果再检查数量、证据引用、重复/改善成立条件和总长度，合格后才附加可读证据详情并保存。页面先展示结论和行动，题目与作答默认折叠，按重点展开。周报不产生记忆候选或自动改写已有 findings，也不新增独立聚合 Job 或问答 UI。

旧版 `weekly_report` 样例与脚本仍是历史/demo 资产；产品路径使用 `weekly_learning_report`。2.0 的上下文、校验、存储和页面已通过 fixture、API smoke、前端构建与浏览器检查，真实 `studyv2-runtime` 的内容质量仍须按 A/B/C 材料验收。
