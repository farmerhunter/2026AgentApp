...
**最终状态（2026-09-02 回读）：** E5 已完成 Final Gate。PR #99、#100 均已合入 `main`，最终 merge commit 为 `3ccefce4971dbb5798c2ea2578a0c5174147785a`，Issue #13 已关闭并进入 Done。下方 `f5d32ee`、旧 Vite/API 进程和历史 job 是 Final Gate 期间保留的分层证据，不代表最终部署版本。阶段 B 展板截图另采用最终 main 与已验收合成结果的隔离回放，未新增 OCR/Hermes 调用。
...
- 2026-09-02 已生成阶段 B 展板程序截图；截图使用最终 main 回放已验收 A/B 结果，不是新的 OCR/Hermes 实验。
- 两批真实 Hermes 链路的题目/作答来自已核对文字 fixture；展板同时展示对应的原创合成练习图片，但不声称历史 Hermes 输出直接来自这两张图片的 OCR。
- 两个 Skill 的记录版本仍为 `e5-wip-0.1`；它们已在 E5 有界范围内通过 Final Gate，但现有质量证据不代表普遍准确率。
- 周报 job 当前按 `student_demo` / `math` 固定查询，尚未参数化。

## 8. PR #99 材料与 PR #100 工程线关系

- PR #99 是 E5 设计、质量和实验材料线，已通过 merge commit `5f4afdf` 合入 `main`。
- PR #100 是 E5 工程实现线，随后同步并保留 #99 的 A/B/C 材料、设计文档和实验结论；最终通过 merge commit `3ccefce` 合入 `main`。
- 最终主线同时包含 E5 工程能力和公开展示材料；Issue #13 的完成说明记录了能力、验证、排除项和 E6 后续边界。
