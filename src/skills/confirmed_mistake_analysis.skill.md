...
6. 当 `finding_type` 不是 `unknown`，且 `evidence_summary` 中存在可观察的学生作答或备注依据时，应提出一条待确认记忆，保留有依据的局部观察；不要把未验证的心理假设或建议当作已发生事实。没有明确证据或判断为 `unknown` 时可为空。接受记忆不等于证明错因、执行建议或取得效果。
