...
| --- | --- | --- | --- | --- |
| 模拟练习 A 图片 | 展示第一次练习及错误线索 | 本项目原创合成练习，不含真实学生信息 | 可用于草图 | 终稿前再次核对题面与展示结果 |
| 模拟练习 B 图片 | 展示第二次练习及作答变化 | 本项目原创合成练习，不含真实学生信息 | 可用于草图 | 终稿前再次核对题面与展示结果 |
| 流程箭头和几何图标 | 表达作品流程与结果分类 | 在 SVG 中自行绘制 | 可用 | 终稿统一线宽和颜色 |
| 导入与确认界面截图 | 关键程序截图 1 | `main@3ccefce` Web UI；原创 A 卷和隔离回放 DB | 已生成 | 保留原始图和展板裁切图 |
| 分析与记忆界面截图 | 关键程序截图 2 | `main@3ccefce` Web UI；已验收 A/B Hermes 输出回放 | 已生成 | 展板图由同页两段未改内容组合，见裁切清单 |
| 学习周报界面截图 | 关键程序截图 3 | `main@3ccefce` Web UI；已验收周报输出回放 | 已生成 | 保留原始图和展板裁切图 |
| 公开体验二维码 | 现场体验入口 | 由最终公开链接生成 | 被阻塞 | 链接确认后生成并用两台设备实测 |
| 字体 | 全板文字 | 草图使用系统中文无衬线字体回退 | 待确认 | 印刷 PDF 前嵌入或转曲，并确认商用许可 |
| 活动统一标识 | 标题区（如要求） | 仅使用主办方正式提供的文件 | 待确认 | 不从搜索图片或第三方页面复制 |
