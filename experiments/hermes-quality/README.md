...
最新：[两批学习故事首轮](learning-story/review.md)已执行 A06/A09/A10 的一批真实文字分析（1 次模型调用）。本题计算和局部判断大体正确，但 A06 再次出现含变量关系的无条件“不等于”表述；按本轮停止条件保留结果，B 批与周报未运行，记忆未接受。未覆盖或替换下面的旧单题基线，不将本次结果宣称为已通过正式 Skill 质量验收。

配套 [两批仿真练习文字稿](two-batch-learning-fixture/README.md)：每批 10 题，含模拟批改，题干/学生作答与人工参考分开。A 卷图片两次内置生成均网络失败，尚无图像或 OCR 结果；文字分析进展见上。

## 此前单题实验归档

以下“本次”等时间措辞指各自归档时点，不否定上方学习故事 A 批的新调用；旧 runner、候选与原始结果保留不变。
