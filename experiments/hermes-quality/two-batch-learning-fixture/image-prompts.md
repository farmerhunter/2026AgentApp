...
状态：A 卷累计三次内置请求均以网络错误结束，没有图片；B 尚未请求。最后一次长等待期间用户中断过会话，不把等待时长当作已知服务端生成时间。随后白底蓝圆小样约34秒成功，说明当时链路可用，但未定位 A 失败的唯一根因；圆点图不作为项目产物维护。文字稿仍是内容权威，图片生成不等于 OCR 或 Hermes 验证通过。
