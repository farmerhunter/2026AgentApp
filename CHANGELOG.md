...
## 2026-09-17 — #122 本次人工智能作品征集说明文档讨论稿

- 新增 `deliverables/2026-ai-works-collection/development-application-draft.md`，按五部分组织作品信息、架构与 AI 能力、开发迭代、应用步骤及验证案例，复用最终海报高清架构图、三组程序截图与 A/B 仿真材料。
- 增强组件部署和调用关系、Hermes Skills 的经验积累与验证闭环，以及 Demo 与真实应用的不同验证目标；明确历史回放、当前真实功能暂停和未来学习反馈能力的边界。
- 更新本次活动清单与 #122 的文档入口，不覆盖旧活动交付物，也不合并旧 PR #103。
- 验证：基本信息 294 字（含标点）；配图及相对文件引用检查、UTF-8 解码、差异空白检查；本轮为纯文档改动，未运行产品测试、调用 OCR/Hermes 或部署 VPS。
- 遗留：讨论稿不是正式 PDF；作者分工、最终参评入口、模型型号与截图仍待核对，PDF 排版和逐页验收待后续完成，#122 保持开放。
...
## 2026-09-11 — 周报 2.0：重点先行与题目依据折叠

- 按 #112 确认的内容设计收敛真实周报：程序提供日期、练习数、错题数、主题与可读题号，Hermes Skill 只生成 1–3 个重点、最多 1 个待观察项和 1–2 个可执行行动。
- 新增 `weekly_learning_report` 2.0 contract 与保存前校验：拒绝未知证据、伪重复、无时间对照的伪进步、缺少判断边界、内部 ID、篇幅或数量超限；无效结果不会进入保存步骤。
- 真实周报页改为总览、重点卡片、待观察和行动结构；题目原文与作答默认收起，按结论查看依据；旧版历史周报继续使用摘要兼容展示，并补充打印隐藏规则。
- 验证：Node 22 下 E5 fixture smoke、Web production build、demo data 140/140 通过；本地浏览器检查新版默认折叠、单项展开和旧周报切换通过。真实 Hermes 内容验收仍需具备 `studyv2-runtime` 的环境执行，fixture 结果不作为 AI 内容质量证明。
- 真实回放补充：在 VPS 私有目录用 DeepSeek `deepseek-v4-pro` 完成 A/B 与 A/B/C 有界回放。修复 2.0 输出前有分析文字时解析器误选嵌套对象的问题，并将周报任务单独设为 low reasoning。A/B 输出通过程序与人工检查；A/B/C 暴露范围绝对化和篇幅波动，程序能拒绝无效结果，但内容 Final Gate 尚未通过。详见 `experiments/hermes-quality/results/2026-09-11-weekly-v2/review.md`。
- 隔离部署复测发现 390/768px 报告列表会撑宽页面、打印会拆开观察卡片，已补充网格收缩和打印防跨页规则。三次 A/B/C 首轮复测为 1 次通过、2 次因模型文案超限被拒绝，旧报告均保持不变；为此给 Skill 增加低于硬门禁的目标预算，并在服务端仅按已有标点收短冗余分句，证据引用、数字、算式和内部标识检查仍使用原始输出复核。
- 最终隔离复测连续 3 次真实 A/B/C 生成均通过程序与人工检查；进一步禁止在顿号处拆开并列算式，并给摘要保留更充足的完整表达空间。1366/768/390px 均无横向滚动，A4 打印保持 2 页、卡片不跨页，行动标题会与第一张行动卡一起换页；无数据任务仍不覆盖历史周报。
- 新增三次真实周报的可读对比文档，逐项列出总览、重点、观察项、行动与 A/B/C 人工核对结论；#115 增加“A/B/C 三卷题目、作答、数学结论、变化判断和证据关联均无误”的明确验收标准。
- 选定第三次周报制作私有终评候选快照，manifest 固定 2.0 contract、Skill 与周报正文 hash；更新 E6 operator 兼容新旧周报并在恢复时核对已验收正文。隔离演练验证了篡改拦截、恢复成功和旧快照兼容，并增加非正式数据根必须指定独立 service/base 的 fail-closed 保护。

## 2026-09-09 — 修正跨周生成周报的页面反馈

- 核对 E5 周报上下文、报告 API、真实页面和 #106：历史 A/B 周报按周期保留，跨周不会自动删除；新生成任务仍只汇总服务器当前自然周。
- 真实周报页在 job 完成后读取领域结果：`no_data` 不再误报“周报已保存”，而是说明当前周无分析数据并继续保留历史周报；报告列表补充明确的起止日期。
- E5 smoke 增加空白未来周回归检查，验证返回 `no_data` 且不会删除已有报告。
- 验证：Node 22.22.2 下 Web build 与 `smoke:e5` 通过，`git diff --check` 通过；系统默认 Node 24 与既有 `better-sqlite3` ABI 不兼容，未为本次改动重装依赖。

## 2026-09-09 — 补读私有 GitHub Project 看板

- 经用户授权后读取 `2026AgentApp Dev` 私有看板的 README、18 个字段、5 个视图和全部 76 个条目，补充状态、优先级及当前终评工作顺序。
- 更新 `docs/project-understanding-2026-09-08.md`：确认 `/demo` 与 `/app` 的看板口径、E1–E6 均为 Done、#18 为唯一 In progress，并移除 Project 权限缺口说明。
- 完整阅读 `docs/quickstart.md`、`docs/v2/development-guide.md` 和本地 E6 讲解指南，并把两份快速指南的定位及当前事实偏差补入接续笔记；实际复跑 demo data 校验为 140/140。
- 本次只读取 GitHub 并更新交接笔记；没有编辑看板、Issue、PR 或仓库远端状态，也没有改动产品实现。

## 2026-09-08 — 项目阅读与 Codex 接续理解

- 阅读本地项目文档、关键实现及 GitHub Issue/PR 与普通评论，交叉核对 V1/V2/V3 边界、E1–E6 历史验收、质量实验和终评状态。
- 新增 `docs/project-understanding-2026-09-08.md`，记录产品主线、业务/工程流程、A/B/C 学习故事、协作方式、分支差异和资料覆盖限制；该文档是阅读快照，不替代已有事实源或建立新规则。
- 验证方式为资料与代码交叉检查、GitHub API 状态核对及文档 diff 检查；未复跑业务测试、调用 OCR/Hermes 或重新验证线上环境，未提交推送或修改业务实现。
- 遗留：Project 看板权限随后已于 2026-09-09 补齐；二进制材料未做逐页视觉审阅；自然周切换、旧材料口径与本地/远端分叉需在对应后续任务处理。

## 2026-08-31 — 换机收尾与遗漏试卷图片补交（#13）

- 补入两张昨天生成、此前只留在本机的第19章六题 PNG，以及实际生成/修正提示词；v1 是有第4题缺陷的初稿，v2 是修正版，都不是新 A/B 卷。
- 更新生图状态：A 累计三次未成功，B 尚未请求；最小蓝圆测试成功，但按用户要求不维护无复用价值的测试图。
- 保留已提交的 E5 设计、两批文字稿、实际接受记忆、三次真实调用及周报原文；换机步骤、未完成项和远端核验记录直接写入 #13 / PR #99，不新增交接规范或系统。
- 本轮只归档与同步，不新增生图/Hermes/OCR 调用、不改产品代码、不合并 main、不部署。私有照片、凭证、数据库和缓存不上传；核对必要文件与图片 hash 后从远端全新取回检查。

## 2026-08-31 — E5 两批记忆复用与周报文字实验完成（#13）

- 用户明确接受 A06/A10 两条实际候选，独立记录原文、来源与实验范围；继续执行 B 批和周报，不重跑 A、不改 Skill、不新增评测设施。
- 新增 B/周报共 2 次真实 Hermes 调用，23,830 tokens；完整故事 3 次调用，32,222 tokens。归档最小输入/输出/用量/快照，并提供未经润色的可读周报。
- B06 有来源地识别重复，B09 保持 unknown，B10 保留正确求截距步骤；周报完成前后比较、限制说明和两项行动。保留错题计数口径、实验术语、一般化措辞问题，不因此反复调参或把结果称为发布版。
- 同步实验入口、材料说明和质量文档；检查接受原文、输入来源、三个步骤结构与引用、数学、可读摘录、链接/格式/敏感内容。仅同一助手自查，不是独立 Final Gate。
- 未执行图片/OCR、产品保存/UI/打印或部署；David 的 E5 工程接线继续按既有交接，现有 #13 / PR #99 保持未完成/草稿。

## 2026-08-31 — E5 学习故事 A 批实测与图片生成受阻（#13）

- 新增 `experiments/hermes-quality/learning-story/`：两份实验 Skill 候选、A 批三题输入、离线结构检查器、真实调用最小证据与质量检查；复用旧 runner，不修改已有失败基线或产品实现。
- A 批真实 Hermes/DeepSeek 调用 1 次、8,392 tokens；三题结构/引用检查通过，A09/A10 局部分析基本符合要求，A06 本题计算正确但一般化“不等于”遗漏零值情况。按停止条件暂停 B/周报，没有接受记忆或伪造产品数据。
- A 卷内置图片生成两次均网络失败，无图片产物，B 卷未请求；完整提示词与失败原因留在 `image-prompts.md`，没有擅自切换额外 API 路径。
- 修正材料说明中“已有 OCR 答案可编辑”的错误假设：当前 E4 只能补缺失答案，红笔混入仍需实测及单独处理。
- 验证：实际输入对照原作答、运行/Skill 快照及结构检查、本地数学数值与反例核对、脚本语法、文档链接和 diff 检查。仅同一会话自查；未做图片/OCR、B 批记忆复用、周报、产品保存/UI 或独立 Final Gate。
